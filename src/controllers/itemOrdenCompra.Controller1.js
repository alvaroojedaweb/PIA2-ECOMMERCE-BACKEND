// src/controllers/itemOrdenCompraController.js
...
exports const getItemsByOrden = async (req, res) => {
  try {
    ...
    const items = await ItemOrdenCompra.findAll({
      where: { orden_compra_id: id_orden },
      include: [Producto]
    });
...
exports const getItemById = async (req, res) => {
  try {
...
// Crear un nuevo item
exports const createItem = async (req, res) => {
  try {
    const { id_cliente, id_orden } = req.params;
    const { producto_id, cantidad } = req.body;
    
    // Verificar que la orden pertenece al cliente y está pendiente
    const orden = await OrdenCompra.findOne({
      where: {
        orden_compra_id: id_orden,
        cliente_id: id_cliente
...
// Actualizar un item
exports const updateItem = async (req, res) => {
  try {
    const { id_cliente, id_orden, id_item } = req.params;
    ...
    // Calcular diferencias para stock y total
    ...
    const diferenciaTotal = nuevoSubtotal - item.subtotal;
    ...
    await orden.update({
      total: orden.total + diferenciaTotal
    });
    
    const itemActualizado = await ItemOrdenCompra.findByPk(id_item, {
      include: [Producto]
    });
...
// Eliminar un item (hard delete)
exports const deleteItem = async (req, res) => {
  try {
    const { id_cliente, id_orden, id_item } = req.params;
    ...
    // Actualizar total de la orden
    await orden.update({
      total: orden.total - item.subtotal
    });
